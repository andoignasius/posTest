	...
	<?php
		if (isset($_GET['kirim'])) {
			for ($i=0; $i < $_rows; $i++) { 
				...
				$odr_brg_id = 'asd12';
				$odr_id = $_GET['id_barang_'.($i + 1)];  
				$odr_jml = $_GET['jumlah_barang_'.($i + 1)];  

					$input = mysql_query ("INSERT INTO `barang_order` (`no`, `id_barang_order`, `id_order`, `id_barang`, `jumlah_barang`) VALUES (NULL, '$odr_brg_id', '', '$odr_id', '$odr_jml')");
				if ($input) {
					echo '<script language="javascript">';
					echo 'alert(" successfully ")';
					echo '</script>';
		...
		function insertBrgOrd( $idBrg, $idOrdBrg, $jmlBrg)
		{
